...
const users = {
    "me": { password: "123", name: "حبيبي", role: "writer" },
    "mona": { password: "456", name: "منى", role: "reader" }
};
...
// تاريخ بداية المشروع (تأكد أن هذا التاريخ هو اليوم الذي تريد أن يبدأ فيه عد "اليوم الأول")
...
function showMonths() {
    ...
    months.forEach((m) => {
        ...
        // قفل الشهور المستقبلية لـ mona فقط
        if (currentUser === "mona" && m.idx > currentMonthNow) {
...
function showDays(monthIdx) {
    ...
    if (currentUser === "mona") {
        // مونا يظهر لها الأيام المسموحة فقط (من 1 إلى اليوم الحالي)
        if (diffDays < 1) {
            showAlert("المفاجأة لم تبدأ بعد! انتظر لليوم الأول ❤️");
            showMonths();
            return;
        }
        ...
        // حسابك (me) يظهر له كل الأيام 1-30 للكتابة مسبقاً
        for (let i = 1; i <= 30; i++) {
...
async function openNote(day) {
    ...
    input.value = "جاري سحب كلمات الحب... 🌸";
    ...
    if (currentUser === "mona") {
        ...
        input.value = serverContent || "لم يكتب لكِ حبيبكِ شيئاً بعد لهذا اليوم.. 🌸";
    } else {
...
async function saveNote() {
    ...
    showAlert("تم حفظ الكلام في السحابة❤️");
}
